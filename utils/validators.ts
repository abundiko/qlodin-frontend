...
export const __validators = {
  ...
  url: z.string().url("invalid url/link"),
  urlOrEmpty: z.string().url("invalid url/link"),
  bio: z
    .string()
    .min(6, "bio too short")
  ...
  bioOrEmpty: z
    .string()
    .max(100, "Bio must be less than 100 characters")
    .refine((str) => str === "" || str.length >= 6, {
      message: "Bio must be either empty or have at least 6 characters",
    }),
};
